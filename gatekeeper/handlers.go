...
func (r *oauthProxy) getRedirectionURL(w http.ResponseWriter, req *http.Request) string {
	...
	default:
		redirect = "http://" + req.Host //r.config.RedirectionURL
	}
...
var tmpl = template.Must(template.ParseFiles("frontend/dist/templates.html"))

func (r *oauthProxy) adminHandler(w http.ResponseWriter, req *http.Request) {
	w.Header().Set(versionHeader, getVersion())
	w.WriteHeader(http.StatusOK)
