...
func (r *oauthProxy) createReverseProxy() error {
	...
	fmt.Println(r.config.BaseURI + r.config.OAuthURI)
	...
	engine.With(proxyDenyMiddleware).Route(r.config.BaseURI+r.config.OAuthURI, func(e chi.Router) {
		...
		e.HandleFunc(authorizationURL, r.oauthAuthorizationHandler) // /oauth/authorize
		e.Get(callbackURL, r.oauthCallbackHandler)                  // /oauth/callback
		e.Get(expiredURL, r.expirationHandler)                      // /oauth/expired
	...
	fmt.Println(docgen.JSONRoutesDoc(engine))
...
func (r *oauthProxy) createUpstreamProxy(upstream *url.URL) (*goproxy.ProxyHttpServer, error) {
	...
	re := regexp.MustCompile(`(?s)<head>(.*)</head>.*<body>(.*)</body>`)
	sideMenuTemplBytes, err := ioutil.ReadFile("frontend/dist/sidemenu.html")
	util.CheckErr(err)
	sideMenuTempl := string(sideMenuTemplBytes)
	submatchall := re.FindAllStringSubmatch(string(sideMenuTempl), -1)

	head := submatchall[0][1]
	body := submatchall[0][2]
	fmt.Println(head)
	fmt.Println(body)
			...
			for {
				...
				case html.EndTagToken:
					...
					if string(tn) == "body" {
						...
						var appNames []NameUuid
						for _, container := range containers {
							if name, ok := container.Labels["udesk_name"]; ok {
								if uuid, ok := container.Labels["udesk_uuid"]; ok {
									if owner, ok := container.Labels["udesk_owner"]; ok {
										appNames = append(appNames, NameUuid{name, uuid, owner})
									}
								}
							}
						...
						sort.Slice(appNames, func(i, j int) bool {
							return appNames[i].Name > appNames[j].Name
						})
