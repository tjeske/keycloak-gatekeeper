...
func NewDockerClient(version string) *DockerClient {
	os.Setenv("DOCKER_API_VERSION", "1.12")

	// Docker HTTP API client
	var httpClient *http.Client
	client, err := client.NewClient(client.DefaultDockerHost, "1.12", httpClient, nil)
	util.CheckErr(err)
...
func (dc *DockerClient) Run(containerName, userName string, args2 map[string]string, dockerRunArgs []string, args []string, cb func()) {
	// defer cb()
	cmdDockerRun := cmd_container.NewRunCommand(dc.dockerCli)

	imageID := "dockertest" //dc.getImageID(containerName, userName, args2, dockerBuildCtx)
	fmt.Println(imageID)
...
func (dc *DockerClient) Create(containerName, userName string, args2 map[string]string, dockerRunArgs []string, args []string, cb func()) {
	// defer cb()
